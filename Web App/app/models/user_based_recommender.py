...
def get_user_based_recommendations(user_encoder, item_encoder, model, df, user_id, age=None, top_n=100):
    if user_id in df['User_ID'].values:
        ...
        region_encoded_series = df.drop_duplicates('product_id_encoded').set_index('product_id_encoded')[
            'region_encoded']
        age_normalized_series = df.drop_duplicates('product_id_encoded').set_index('product_id_encoded')[
            'age_normalized']
        ...
        if age is not None:
            ...
            avg_interaction = age_filtered_items.groupby('product_id_encoded')[
                'interaction_strength'].mean().reset_index()
            ...
            top_recommendations = [(row['product_id_encoded'], row['interaction_strength']) for _, row in
                                   recommendations.iterrows()][:top_n]
        else:
